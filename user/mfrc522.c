 ...
 */
void ICACHE_FLASH_ATTR PCD_WriteRegister(uint8_t reg, uint8_t value)
{
  /* Select SPI Chip MFRC522 */
  ...
  //(void) m_SPI.write(value);
	//spi_mast_byte_write(HSPI, reg & 0x7E);
	//spi_mast_byte_write(HSPI, value);
  platform_spi_send_recv(HSPI, reg & 0x7E, 0);
  platform_spi_send_recv(HSPI, value, 1);

  /* Release SPI Chip MFRC522 */
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void ICACHE_FLASH_ATTR PCD_WriteRegister2(uint8_t reg, uint8_t count, uint8_t *values)
{
  /* Select SPI Chip MFRC522 */
	//GPIO_OUTPUT_SET(15, 0);
  ...
  for (uint8_t index = 0; index < count; index++)
  {
    //(void) m_SPI.write(values[index]);
		//spi_mast_byte_write(HSPI, values[index]);
		//
		if(index == count-1)
		{
			close = true;
		}
  	platform_spi_send_recv(HSPI, values[index], close);
  }
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
uint8_t ICACHE_FLASH_ATTR PCD_ReadRegister(uint8_t reg)
{
  uint8_t value;
  /* Select SPI Chip MFRC522 */
	//GPIO_OUTPUT_SET(15, 0);
  ...
  //(void) m_SPI.write(0x80 | reg);
	//spi_mast_byte_write(HSPI, 0x80 | reg);
  platform_spi_send_recv(HSPI, 0x80 | reg, false);

  // Read the value back. Send 0 to stop reading.
  //value = m_SPI.write(0);
	//TODO CHECK THIS
	//spi_mast_byte_write(HSPI, 0);
  value = platform_spi_send_recv(HSPI, 0, true);
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void ICACHE_FLASH_ATTR PCD_ReadRegister2(uint8_t reg, uint8_t count, uint8_t *values, uint8_t rxAlign)
{
	rxAlign = 0;
  if (count == 0) { return; }
  ...
  //(void) m_SPI.write(address);   // Tell MFRC522 which address we want to read
	//spi_mast_byte_write(HSPI, address);
  platform_spi_send_recv(HSPI, address, false);
  ...
  while (index < count)
  {
    if ((index == 0) && rxAlign) // Only update bit positions rxAlign..7 in values[0]
    {
      ...
      //uint8_t value = m_SPI.write(address);
			//TODO FIX
			//spi_mast_byte_write(HSPI, address);
  		uint8_t value = platform_spi_send_recv(HSPI, address, false);
    ...
    else
    {
      ...
      //values[index] = m_SPI.write(address);
			//TODO FIX
			//spi_mast_byte_write(HSPI, address);
  		values[index] = platform_spi_send_recv(HSPI, address, false);
    }
  ...
  //values[index] = m_SPI.write(0); // Read the final byte. Send 0 to stop reading.
	//TODO fix
	//spi_mast_byte_write(HSPI, 0);
  values[index] = platform_spi_send_recv(HSPI, 0, true);
 ...
 */
void ICACHE_FLASH_ATTR PCD_SetRegisterBits(uint8_t reg, uint8_t mask)
{
 ...
 */
void ICACHE_FLASH_ATTR PCD_ClrRegisterBits(uint8_t reg, uint8_t mask)
{
  uint8_t tmp = PCD_ReadRegister(reg);
 ...
 */
uint8_t ICACHE_FLASH_ATTR PCD_CalculateCRC(uint8_t *data, uint8_t length, uint8_t *result)
{
 ...
 */
void ICACHE_FLASH_ATTR PCD_Init()
{
	gpio_init();
  spi_init();
	//set CS high
	...
	/* Initialize SPI 1 */
	//spi_master_init(HSPI, 0, 0, 0, 0); 
  //spi.begin(15);
	//spi.setDataMode(SPI_MODE0);
	//spi.setBitOrder(MSBFIRST);

  /* Initialize GPIO5 for setting the RST line */
	//Set GPIO5 to output mode
	//WTF! GPIO4 and GPIO5 are reversed...
 ...
 */
void ICACHE_FLASH_ATTR PCD_AntennaOn()
{
  uint8_t value = PCD_ReadRegister(TxControlReg);
	//os_printf("antenna val %x\r\n", value);
  if ((value & 0x03) != 0x03)
  {
 ...
 */
uint8_t ICACHE_FLASH_ATTR PCD_TransceiveData(uint8_t *sendData,
                                    uint8_t sendLen,
                                    uint8_t *backData,
                                    uint8_t *backLen,
                                    uint8_t *validBits,
 ...
 * CRC validation can only be done if backData and backLen are specified.
 */
uint8_t ICACHE_FLASH_ATTR PCD_CommunicateWithPICC(uint8_t command,
                                         uint8_t waitIRq,
                                         uint8_t *sendData,
                                         uint8_t sendLen,
                                         uint8_t *backData,
 ...
 */
uint8_t ICACHE_FLASH_ATTR PICC_RequestA(uint8_t *bufferATQA, uint8_t *bufferSize)
{
 ...
 * Beware: When two PICCs are in the field at the same time I often get STATUS_TIMEOUT - probably due do bad antenna design.
 */
uint8_t ICACHE_FLASH_ATTR PICC_WakeupA(uint8_t *bufferATQA, uint8_t *bufferSize)
{
 ...
 * Beware: When two PICCs are in the field at the same time I often get STATUS_TIMEOUT - probably due do bad antenna design.
 */
uint8_t ICACHE_FLASH_ATTR PICC_REQA_or_WUPA(uint8_t command, uint8_t *bufferATQA, uint8_t *bufferSize)
{
  uint8_t validBits;
  uint8_t status;
  ...
  if (status != STATUS_OK)
  {
		//os_printf("returning status %d\r\n",status);
    return status;
  }

  if ((*bufferSize != 2) || (validBits != 0))
  {   // ATQA must be exactly 16 bits.
		//os_printf("returning error status\r\n");
    //return STATUS_ERROR;
    return 202;
  }

	//os_printf("returning status OK\r\n");
  return STATUS_OK;
 ...
 */
uint8_t ICACHE_FLASH_ATTR PICC_Select(Uid *uid, uint8_t validBits)
{
  bool uidComplete;
  bool selectDone;
  bool useCascadeTag;
 ...
 */
uint8_t ICACHE_FLASH_ATTR PICC_HaltA()
{
  uint8_t result;
  uint8_t buffer[4];
 ...
 */
uint8_t ICACHE_FLASH_ATTR MIFARE_Read(uint8_t blockAddr, uint8_t *buffer, uint8_t *bufferSize)
{
 ...
 */
uint8_t ICACHE_FLASH_ATTR MIFARE_Decrement(uint8_t blockAddr, uint32_t delta)
{
 ...
 */
uint8_t ICACHE_FLASH_ATTR MIFARE_Increment(uint8_t blockAddr, uint32_t delta)
{
 ...
 */
uint8_t ICACHE_FLASH_ATTR MIFARE_Restore(uint8_t blockAddr)
{
 ...
 */
uint8_t ICACHE_FLASH_ATTR MIFARE_TwoStepHelper(uint8_t command, uint8_t blockAddr, uint32_t data)
{
  uint8_t result;
 ...
 */
uint8_t ICACHE_FLASH_ATTR MIFARE_Transfer(uint8_t blockAddr)
{
  uint8_t cmdBuffer[2]; // We only need room for 2 bytes.
 ...
 */
uint8_t ICACHE_FLASH_ATTR PCD_MIFARE_Transceive(uint8_t *sendData, uint8_t sendLen, bool acceptTimeout)
{
  uint8_t result;
 ...
 */
uint8_t ICACHE_FLASH_ATTR PICC_GetType(uint8_t sak)
{
 ...
 */
char* ICACHE_FLASH_ATTR PICC_GetTypeName(uint8_t piccType)
{
 ...
 */
char* ICACHE_FLASH_ATTR GetStatusCodeName(uint8_t code)
{
 ...
 */
void ICACHE_FLASH_ATTR MIFARE_SetAccessBits(uint8_t *accessBitBuffer,  
                                   uint8_t g0,                
                                   uint8_t g1,                
                                   uint8_t g2,                
                                   uint8_t g3)
 ...
 */
bool ICACHE_FLASH_ATTR PICC_IsNewCardPresent(void)
{
 ...
 */
bool ICACHE_FLASH_ATTR PICC_ReadCardSerial(void)
{
