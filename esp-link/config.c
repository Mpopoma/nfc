...
FlashConfig flashDefault = {
  ...
  .api_key      = "",
  .nfc_counter  = 3,
	.nfc_url      = "http://example.com/api/rfid\0",
	.nfc_device_id = "device_id\0",
	.nfc_device_secret = "secret\0",
	.sys_descr 	  = "\0",
  .rx_pullup	  = 1,  
...
bool ICACHE_FLASH_ATTR configRestore(void) {
  ...
  if (flash_pri < 0) {
    ...
    char hostname[16];
    os_strcpy(hostname, "happy-bubbles-nfc-");
